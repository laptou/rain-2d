...
namespace Rain.Commands
{
    public sealed class SubdividePathCommand : LayerCommandBase<Path>
    {
        ...
        public SubdividePathCommand(long id, Path target, int edge) : base(id, new[] {target}) { Edge = edge; }
        ...
        {
            var nodes = GeometryHelper.NodesFromInstructions(Targets[0].Instructions).ToList();
                ...
                prev.OutgoingControl == null)
            {
                ...
                _original = new[] {prev.Position, next.Position};
                     ...
                     next.IncomingControl != null)
            {
                ...
                var (c, s1, s2) = CurveAnalysis.DeCasteljau.Subdivide(0.5f, prev.Position, p1, p2, next.Position);

                _original = new[] {prev.Position, p1, p2, next.Position};
            ...
            else if (next.IncomingControl != null && prev.OutgoingControl == null ||
                     next.IncomingControl == null && prev.OutgoingControl != null)
            {
                var p = (next.IncomingControl ?? prev.OutgoingControl).Value;

                var (c, s1, s2) = CurveAnalysis.DeCasteljau.Subdivide(0.5f, prev.Position, p, next.Position);

                _sFlag = next.IncomingControl == null;

                _original = new[] {prev.Position, p, next.Position};
        ...
        public override void Undo(IArtContext context)
        {
            ...
            {
                prev = new PathNode(prev.Index, _original[0], prev.IncomingControl, _original[1], prev.FigureEnd);

                next = new PathNode(next.Index - 1, _original[3], _original[2], next.OutgoingControl, next.FigureEnd);
            }
            else
            {
                if (_sFlag)
